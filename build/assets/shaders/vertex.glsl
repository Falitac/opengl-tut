...
void main() {
  ...
  fragmentColor.x = abs(cos(exp(position.z*position.y)));
  fragmentColor.y = abs(sin((exp(position.y))));
  fragmentColor.z = abs(cos((exp(position.z + 30.f) + 0.3*position.z*position.z)));
}